/**
 * Pure discovery surface ranking helpers.
 *
 * Derives factual predicates and curated entry lists from registry metadata for
 * growth/discovery rails. Nothing here touches the network — given the same
 * entry fields the output is deterministic.
 *
 * The public surface (`growth-surface-rules.ts` / `@/lib/growth-surface-rules`)
...
function latestEntryDate<T extends GrowthSurfaceEntry>(entries: T[], field: keyof T) {
...
export function hasInstallSurface(entry: GrowthSurfaceEntry) {
  return Boolean(entry.installCommand || entry.downloadUrl || entry.configSnippet);
}

export function hasSafeInstallSignal(entry: GrowthSurfaceEntry) {
  return entry.downloadTrust === "first-party" || entry.packageVerified === true;
}
...
export function buildDiscoverySurfaceLists<T extends GrowthSurfaceEntry>(
  ...
  const recentlyUpdated = [...entries]
    ...
    .sort((left, right) => String(right.repoUpdatedAt).localeCompare(String(left.repoUpdatedAt)))
    .slice(0, limit);
  const recentlyVerified = [...entries]
    ...
    .sort(
      (left, right) =>
        dateValue(recentVerificationDate(right)) - dateValue(recentVerificationDate(left)),
    )
    .slice(0, limit);
  ...
  const popularBySourceSignals = [...entries]
    .filter((entry) => typeof entry.githubStars === "number" && entry.githubStars > 0)
