/**
 * Pure content-quality and source-health report builders.
 *
 * This module holds the deterministic scoring layer used to publish the
 * content-quality, source-health, and quality-prompt artifacts. Nothing here
 * touches the filesystem or network — given the same entry snapshot the output
 * is byte-stable, which is what keeps published report hashes deterministic.
 *
 * The public package surface (`quality.js` / `@heyclaude/registry/quality`)
...
export function normalizeBodyForDuplicateCheck(entry) {
...
export function scoreFreshness(entry, referenceDate = new Date()) {
...
export const SOURCE_HEALTH_RISK_CATEGORIES = new Set([
  "mcp",
  "hooks",
  "skills",
  "statuslines",
...
export function isRiskBearingSourceCategory(category) {
  return SOURCE_HEALTH_RISK_CATEGORIES.has(clean(category));
}

export function hasMeaningfulNotes(value) {
...
export function derivePackageTrust(entry) {
...
export function buildSourceHealthCategoryBreakdown(rows) {
