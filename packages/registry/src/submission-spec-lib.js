/**
 * Pure submission-spec builders.
 *
 * This module holds the deterministic field-model and intake-spec layer used
 * to describe HeyClaude submission forms. Nothing here touches the network —
 * given the same category and options the output is byte-stable.
 *
 * The public package surface (`submission-spec.js` /
 * `@heyclaude/registry/submission-spec`) re-exports everything below so
...
export const RISK_BEARING_SUBMISSION_CATEGORIES = new Set([
  "mcp",
  "hooks",
  "skills",
  "commands",
...
export function fieldFor(id, required = false) {
...
export function submitUrlForOrigin(origin) {
  if (!origin) return "";
  try {
