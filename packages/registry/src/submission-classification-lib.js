/**
 * Pure submission classification helpers.
 *
 * Detects commercial tool/app listing signals, MCP server submissions, and
 * missing tool review fields from submission payloads. Nothing here touches the
 * network — given the same fields the output is deterministic.
 *
 * The public surface (`submission-classification.js` /
 * `@heyclaude/registry/submission-classification`) re-exports everything below
...
/**
 * Collect commercial tool/app listing signals from structured fields and text.
 *
...
/**
 * Detect MCP server submissions from fields and free text.
 *
 * @param {Record<string, unknown>} [fields]
 * @param {unknown} [text]
 * @returns {boolean}
...
/**
 * Return true when a submission looks like a commercial tool/app listing.
 *
 * @param {Record<string, unknown>} [fields]
 * @param {unknown} [text]
 * @returns {boolean}
...
/**
 * Report tool listing review fields that are still absent from a payload.
 *
 * @param {Record<string, unknown>} [fields]
 * @returns {string[]}
 */
