...
also runs a hosted remote MCP surface. It requires operator-configured
credentials rather than a plain public URL, so follow the current hosted setup in
the [LoopOver docs](https://loopover.ai/docs) instead of pasting an endpoint
directly.
